    ...
    private void editVideoData(List<MediaCodecInfo> mediaCodecInfos, List<MediaCodec> decodeList,
        ...
        while (!outputDone) {
            ...
            if (!inputDone) {
                ...
                Log.e("libiao", "---解码器 是否可用  " + inputIndex);
                if (inputIndex >= 0) {
                    /*说明解码器有可用的ByteBuffer*/
            ...
            while (!decodeOutputDone || !encodeDone) {
                ...
                Log.e("libiao", "  解码器出来的index   " + outputIndex);
                if (outputIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (doRender && isUseful) {
                         ...
                         * 并且对时间戳 进行重写
                         * */
                        Log.e("libiao", "---卡主了？ 一  " + decodeOutputInfo.size);
                        ...
                        Log.e("libiao", "---在编码画面帧的时候，重置时间戳===" + encodeInputTimeStamp);
                        inputSurface.setPresentationTime(encodeInputTimeStamp * 1000);
                        inputSurface.swapBuffers();

                    } else {
                        Log.e("libiao", "---解码出来的视频有问题=== " + doRender + "   " + isUseful);
                    }
                    ...
                    if ((decodeOutputInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                        ...
                        if (curVideoIndex < mediaCodecInfos.size()) {
                            ...
                            isChangeVideo = true;
                            isNextStep = true;
                            Log.e("libiao", "---更换分离器 and 解码器---==");
                        } else {
                            /*没有数据了 就给编码器发送一个结束的标志位*/
                            videoEncoder.signalEndOfInputStream();
                            inputDone = true;
                            Log.e("libiao", "---所有视频都解码完成了 告诉编码器 可以结束了---==");
                        }
                    }
                }
                /*
                ...
                while (!encodeDone) {
                    ...
                    if (encodeOutputState == MediaCodec.INFO_TRY_AGAIN_LATER) {
                        ...
                        if (encodeOutputInfo.size != 0) {
                            Log.e("libiao", "--写入混合器的数据----presentationTime===" + encodeOutputInfo.presentationTimeUs + "===size===" + encodeOutputInfo.size + "----flags==" + encodeOutputInfo.flags);
        ...
        Log.e("libiao", "---视频编码完成---视频编码耗时-==" + (end - start));
    }
